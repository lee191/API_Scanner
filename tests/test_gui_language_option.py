...
class GuiLanguageOptionTests(unittest.TestCase):
    ...
    def test_sensitive_tab_falls_back_to_hardcoded_findings_when_sensitive_is_empty(self) -> None:
        ...
        try:
            ...
            self.assertEqual(window.sensitive_table.item(0, 1).text(), "backup@example.com")
        finally:
            window.close()
    ...
    def test_api_table_uses_accessible_api_fallback(self) -> None:
        window = DiscoveryWindow()
        try:
            window._rebuild_tables(_sample_result())

            self.assertEqual(len(window.api_rows), 1)
            self.assertEqual(window.api_rows[0][3], "/api/users")
            self.assertEqual(window.api_rows[0][5], "https://example.com/api/users")
        finally:
            window.close()

    def test_page_table_uses_accessible_page_fallback(self) -> None:
        window = DiscoveryWindow()
        try:
            window._rebuild_tables(_sample_result())

            self.assertEqual(len(window.page_rows), 1)
            self.assertEqual(window.page_rows[0][4], "/about")
            self.assertEqual(window.page_rows[0][6], "https://example.com/about")
        finally:
            window.close()

    def test_summary_cards_fall_back_to_visible_rows(self) -> None:
        window = DiscoveryWindow()
        try:
            result = _sample_result()
            result["summary"] = {"js_fetched": 0, "page_count": 0, "api_count": 0}

            window._update_for_selected_result(result)

            self.assertEqual(window.card_page_value.text(), "1")
            self.assertEqual(window.card_api_value.text(), "1")
        finally:
            window.close()
